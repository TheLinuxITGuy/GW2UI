...
GW2UI.GetGlobalEnv = function()
  if GW2UI.GetExpansion() == 'vanilla' then
    return getfenv(0)
  else
...
GW2UI.GetColorGradient = function(perc)
  ...
  if not gradientcolors[index] then
    ...
    local r = GW2UI.round(r1 + (r2 - r1) * perc, 4)
    local g = GW2UI.round(g1 + (g2 - g1) * perc, 4)
    local b = GW2UI.round(b1 + (b2 - b1) * perc, 4)
    local h = GW2UI.rgbhex(r,g,b)
...
GW2UI.HookScript = function(f, script, func)
...
GW2UI.HookAddonOrVariable = function(addon, func)
...
local hooks = {}
GW2UI.hooksecurefunc = function(name, func, append)
...
GW2UI.SanitizePattern = function(pattern)
...
local capture_cache = {}
GW2UI.GetCaptures = function(pat)
...
GW2UI.cmatch = function(str, pat)
  -- read capture indexes
  local a,b,c,d,e = GW2UI.GetCaptures(pat)
  local _, _, va, vb, vc, vd, ve = string.find(str, GW2UI.SanitizePattern(pat))
...
local timer
GW2UI.QueueFunction = function(a1,a2,a3,a4,a5,a6,a7,a8,a9)
  if not timer then
...
GW2UI.strsplit = function(delimiter, subject)
...
GW2UI.AddBorder = function(frame, inset, color)
  if not frame then return end
  if frame.GW2UI_border then return frame.GW2UI_border end
  ...
  if not frame.GW2UI_border then
    frame.GW2UI_border = CreateFrame("Frame", nil, frame)
    frame.GW2UI_border:SetPoint("TOPLEFT", frame, "TOPLEFT", (left or -inset), (top or inset))
    frame.GW2UI_border:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", (right or inset), (bottom or -inset))
    frame.GW2UI_border:SetBackdrop(border)

    if color then
      frame.GW2UI_border:SetBackdropBorderColor(color.r, color.g, color.b, 1)
    end
  end

  return frame.GW2UI_border
end

GW2UI.round = function(input, places)
...
GW2UI.Abbreviate = function(number, eachk)
  local sign = number < 0 and -1 or 1
  number = math.abs(number)

  if number > 1000000 then
    return GW2UI.round(number/1000000*sign,2) .. "m"
  elseif not eachk and number > 10000 then
    return GW2UI.round(number/1000*sign,2) .. "k"
...
GW2UI.TimeConvert = function(remaining)
