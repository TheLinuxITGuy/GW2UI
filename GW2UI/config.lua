...
settings.title.text:SetText(T["GW2UI Options"])
...
settings.okay:SetScript("OnClick", function()
  ...
  for k, v in pairs(current_config) do
    ...
    if current_config[k] ~= GW2UI_config[k] then
      reload = true
    end
...
settings.load = function(self)
  ...
  local gui = {}
  for title, module in pairs(GW2UI.mods) do
  ...
  local yoff = 25
  local entrysize = 25
  for category, entries in GW2UI.spairs(gui) do
    ...
    for title, module in GW2UI.spairs(entries) do
...
settings:SetScript("OnShow", function()
  ...
  for k, v in pairs(GW2UI_config) do
    current_config[k] = v
  end
...
-- Add "GW2UI Settings" Button to the Game Menu
...
advanced:SetText(T["GW2UI Options"] .. "|cffffff00*")
