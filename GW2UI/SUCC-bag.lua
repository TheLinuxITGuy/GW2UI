...
local T = GW2UI.T
local gw2artbag = "Interface\\AddOns\\GW2UI\\img\\bag\\"
...
local function SlotFrameSetup(frame)
	...
	frame.slotFrame:SetBackdrop({
		bgFile = gw2artbag .. 'marble',
		edgeFile = gw2artbag .. 'BagSlotFrame',
		tile = true,
		tileSize = 128,
		edgeSize = 32,
		insets = {
...
local function ItemCreate(name, parent)
	...
	button:SetNormalTexture(gw2artbag .. 'bagitembackdrop.tga')
	btnNormal = button:GetNormalTexture()
	btnNormal:SetHeight(32)
	btnNormal:SetWidth(32)
	button.bg = button:CreateTexture(nil, 'BACKGROUND')
	--button.bg:SetTexture[[Interface\PaperDoll\UI-Backpack-EmptySlot]]
	button.bg:SetTexture(gw2artbag .. "bagitembackdrop.tga")
...
local function PrepareBank(frame)
	...
	for i = 1, NUM_BANKBAGSLOTS, 1 do
		...
		frame.bank.slotFrame.slot[i]:SetNormalTexture(gw2artbag .. 'Slot')
...
for i = 1, NUM_BAG_SLOTS, 1 do
	...
	SUCC_bag.slotFrame.slot[i]:SetNormalTexture(gw2artbag .. 'Slot')
