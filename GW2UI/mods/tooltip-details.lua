...
local T = GW2UI.T
local round = GW2UI.round
local rgbhex = GW2UI.rgbhex
local Abbreviate = GW2UI.Abbreviate
...
module.enable = function(self)
  ...
  statusbar:SetScript("OnUpdate", function()
    ...
    if hpmax > 100 or (round(hpmax/100*hp) ~= hp) then
      ...
      rhp, rhpmax, estimated = GW2UI.libhealth:GetUnitHealthByName(this.name, this.level, tonumber(hp), tonumber(hpmax))
    end
